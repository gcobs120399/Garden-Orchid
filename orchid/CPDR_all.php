...
if(isset($_GET["action"])&&($_GET["action"]=="delete")){
  $query_delMember = "DELETE FROM `history` WHERE `h_id`=".$_GET["id"];
  mysql_query($query_delMember);
  //重新導向回到主畫面
  header("Location: CPDR_all.php");
}
//選會員
...
//選取會員的花資料
$query_RecHistory = "SELECT * FROM `history` WHERE `h_username`='".$_SESSION["loginMember"]."'";
...
//本頁開始記錄筆數 = (頁數-1)*每頁記錄筆數 ORDER BY `h_on` DESC WHERE `h_username`='".$_SESSION["loginMember"]."'
$startRow_records = ($num_pages -1) * $pageRow_records;
//未加限制顯示筆數的SQL敘述句
$query_RecHistory = "SELECT * FROM `history` WHERE `h_username`='".$_SESSION["loginMember"]."'ORDER BY `h_on` DESC";
...
<table width="90%" border="1px" align="center" cellpadding="4" cellspacing="0">
  <tr>
    <td class="tdbline"><table width="100%" border="1px" cellspacing="0" cellpadding="10">
      <tr valign="top">
        <td class="tdrline"><p class="title"><?php echo $row_RecMember["m_username"];?> 您的所有作物歷史紀錄</p>
      ...
      <?php while($row_RecHistory=mysql_fetch_assoc($RecHistory)){ ?>
            <tr>
              <td width="5%" align="center" bgcolor="#FFFFFF"><p><a href="CM_update.php?id=<?php echo $row_RecHistory["h_id"];?>">施工中</a><br>
